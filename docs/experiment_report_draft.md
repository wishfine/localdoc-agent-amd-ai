...
所有数据标记为 `is_simulated=True`，`measurement_type="simulated"`。unavailable 后端（GPU/NPU 未检测到）直接跳过，不参与性能汇总和图表。结果写入 `results/latency_results.csv`、`results/backend_results.csv` 和 `results/resource_usage.csv`。
