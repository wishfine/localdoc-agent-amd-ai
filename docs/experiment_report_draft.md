...
原始系统使用抽取式回答生成（AnswerGenerator 的 extractive 模式），优点是轻量、稳定、无外部依赖。为体现本地 AI 推理能力，本系统额外接入了 Qwen2.5-0.5B-Instruct 本地语言模型（Hugging Face Transformers），作为可选的生成后端。

**模型信息**：
- 模型：Qwen2.5-0.5B-Instruct（0.5B 参数，Apache-2.0 许可证）
