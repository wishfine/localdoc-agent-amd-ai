...
def _build_agent(simulate_npu: bool = False):
    """Create a LocalDocAgent with optional simulated NPU."""
    ...
    if simulate_npu:
        try:
            ...
            logger.info("使用 SimulatedNPUBackend")
        except ImportError:
            logger.warning("SimulatedNPUBackend 不可用，回退到 CPU")

    return LocalDocAgent(backend=backend)


def _get_scheduler_instance(simulate_npu: bool = False):
    """Create a scheduler instance, optionally with simulated NPU."""
    ...
    return HeterogeneousScheduler(backends=backends)


def _get_backend_status(scheduler) -> str:
    """Format backend status as a readable string."""
    lines = []
    for name, backend in scheduler.backends.items():
        status = "✅ 可用" if backend.is_available() else "❌ 不可用"
        backend_name = getattr(backend, "name", type(backend).__name__)
        lines.append(f"- **{name}** ({backend_name}): {status}")
    return "\n".join(lines) if lines else "未检测到后端。"
...
def _format_schedule_report(report: Dict[str, Dict[str, str]]) -> str:
    ...
    for task_type, info in report.items():
        rows.append(f"| `{task_type}` | **{info['backend']}** | {info['reason']} |")
    return header + "\n".join(rows)
...
def create_app():
    ...
    # Module-level state
    _agent = None
    _scheduler = _get_scheduler_instance(simulate_npu=False)
    _ingested_count = 0

    def _ensure_agent(simulate_npu: bool):
        nonlocal _agent, _scheduler
        _agent = _build_agent(simulate_npu=simulate_npu)
        _scheduler = _get_scheduler_instance(simulate_npu=simulate_npu)
        return _agent
    ...
    def handle_query(question: str, simulate_npu: bool):
        """Handle user query against the knowledge base."""
        ...
        try:
            ...
            return answer + latency_info, sources
    ...
    def get_system_info(simulate_npu: bool):
        ...
        scheduler = _get_scheduler_instance(simulate_npu=simulate_npu)
        backend_status = _get_backend_status(scheduler)
    ...
    def run_benchmark(simulate_npu: bool):
        ...
        for entry in log:
            lines.append(
                ...
                f"{entry['elapsed_seconds']:.4f} |"
            )

        lines.append(
            "\n> ⚠️ 注意: 以上为 CPU fallback + simulated 后端的结果，"
            "不代表真实 AMD AI MAX+ 硬件性能。"
        )
        return "\n".join(lines)
...
> **异构计算课程项目** | 面向 AMD 锐龙 AI MAX+ 平台的本地知识库智能体
>
> 支持 CPU / GPU / NPU 多后端异构调度。当前运行在 CPU fallback 模式。
            """
        )
        ...
        with gr.Row():
            simulate_npu_toggle = gr.Checkbox(
                ...
                value=False,
                info="开启后使用 SimulatedNPUBackend 模拟 NPU 加速效果（仅用于演示）",
            )
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("📄 文档上传"):
                ...
                upload_status = gr.Textbox(
                    label="加载状态", lines=8, interactive=False,
                )
                upload_btn.click(
                    fn=handle_upload,
            ...
            with gr.TabItem("🔍 知识库查询"):
                ...
                with gr.Row():
                    answer_output = gr.Textbox(
                        label="回答", lines=10, interactive=False,
                    )
                    sources_output = gr.Textbox(
                        label="引用来源", lines=10, interactive=False,
                    )
                query_btn.click(
                    fn=handle_query,
            ...
            with gr.TabItem("⚡ 基准测试"):
                gr.Markdown("运行快速基准测试，查看各任务的后端分配和执行耗时。")
